...
void rowBatchSobel(unsigned char *s, unsigned char *t, int *mask_d, int H, int W, int C, int y, int ch,
        int x) {
    double val, totalCh;
    int Ch, mask_idx;
...
void sobel(unsigned char* s, unsigned char* t, unsigned H, unsigned W, unsigned C, int img_size) {
    ...
    mask_d = (int*)malloc(mask_size);
    memcpy(mask_d, mask, mask_size);

    unsigned char *sd;
    sd = (unsigned char*)malloc(img_size);
    memcpy(sd, s, img_size);

    for (int ch = 0; ch < C; ++ch) {
        for (int y = 0; y < H; ++y) {
            for (int x = 0; x < W; ++x) {
                rowBatchSobel(s, t, mask_d, H, W, C, y, ch, x);
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    dst_img = (unsigned char*)malloc(img_size);
