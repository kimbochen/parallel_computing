...
int main(int argc, char** argv) {
    ...
    auto dist_sqr = [&](int i, int j) -> double {
        double dx = qx[i] - qx[j];
        double dy = qy[i] - qy[j];
        double dz = qz[i] - qz[j];
    ...
    for (int step = 0; step <= param::n_steps; step++) {
        ...
        min_dist = std::min(min_dist, sqrt(dist_sqr(planet, asteroid)));
    }
    ...
    for (int step = 0; step <= param::n_steps; step++) {
        ...
        if (dist_sqr(planet, asteroid) < param::planet_radius * param::planet_radius) {
            hit_time_step = step;
            break;
        }
    }
